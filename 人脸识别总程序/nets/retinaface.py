...
class ClassHead(nn.Module):
    def __init__(self,inchannels=512,num_anchors=2):
        ...
        self.conv1x1 = nn.Conv2d(inchannels,self.num_anchors*2,kernel_size=(1,1),stride=1,padding=0)  #1x1的卷积

    def forward(self,x):
        out = self.conv1x1(x)
        out = out.permute(0,2,3,1).contiguous()  #因为pytorch上通道数是在前面，所以把通道数移到最后
        
        return out.view(out.shape[0], -1, 2) #第一维度是batch_size第二维度就是所有的先验框第三个维度就是每个先验框内部是否包括人脸（包含人脸的概率）
...
class RetinaFace(nn.Module):
    def __init__(self, cfg = None, pretrained = False, mode = 'train'):
        ...
        self.body = _utils.IntermediateLayerGetter(backbone, cfg['return_layers'])  #获取主干特征提取网络的三个有效特征层
    ...
    def forward(self,inputs):
        ...
        #         C5  20, 20, 256
        #-------------------------------------------#
        out = self.body.forward(inputs)   #进行前向传递
        ...
        #         output3  20, 20, 64
        #-------------------------------------------#
        fpn = self.fpn.forward(out)    #将传递的结果放入FPN当中

        feature1 = self.ssh1(fpn[0])  #应用ssh对3个fpn进行处理
