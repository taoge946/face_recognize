...
class Face_Register:
    ...
    def pre_work_mkdir(self):#没有就新建一个文件夹
    ...
    def pre_work_del_old_face_folders(self):
        ...
        for i in range(len(folders_rd)):
            shutil.rmtree(self.path_photos_from_camera+folders_rd[i])#递归删除文件夹下的所有子文件夹和子文件。
        if os.path.isfile("data/features_all.csv"):   #每次给人脸拍照都运行重新构建csv人脸特征数据集，这样相当于每次录入的时候从新检查一遍库
                                                      #不需要的人脸可以直接删除文件夹
            os.remove("data/features_all.csv")

    # 如果有之前录入的人脸, 在之前 person_x 的序号按照 person_x+1 开始录入 / Start from person_x+1
    
    def check_existing_faces_cnt(self):  #取出已经存了的人脸后的数字（序号），并告诉self目前存了多少个人
        if os.listdir("data/data_faces_from_camera/"):
            ...
            for person in person_list:
                person_num_list.append(int(person.split('_')[0]))#[-1]的意思就是最后一个，就是取数字
    ...
    def process(self, stream):
        ...
        self.check_existing_faces_cnt()#看看文件夹中已经有几个图像了

        while stream.isOpened():     #如果开着摄像头
